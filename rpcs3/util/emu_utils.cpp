...
std::shared_ptr<CPUDisAsm> make_disasm(const cpu_thread* cpu, shared_ptr<cpu_thread> handle)
{
	...
	switch (cpu->get_class())
	{
	case thread_class::ppu: result = std::make_shared<PPUDisAsm>(cpu_disasm_mode::interpreter, cpu->sudo_base_addr ? cpu->sudo_base_addr : vm::g_sudo_addr); break;
	...
	case thread_class::rsx: result = std::make_shared<RSXDisAsm>(cpu_disasm_mode::interpreter, cpu->sudo_base_addr ? cpu->sudo_base_addr : vm::g_sudo_addr, 0, cpu); break;
